...
`mikofo-aoc` that, given a day and input, will
provide the solution.

```
mikofo-aoc 3 my_input.txt
```
...
`mikofo-aoc-bench` that does not rely on `click` but is less
...
```
# with plain poetry
poetry run mikofo-aoc DAY PATH_TO_INPUT

# with poetry shell activated (or if you just installed the distribution)
mikofo-aoc DAY PATH_TO_INPUT

# example
mikofo-aoc 2 inputs/day02.txt
```
